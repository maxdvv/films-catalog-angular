...
export class FilmService {
  ...
  constructor(
    ...
    @Inject(MID_IMG_PATH) public midImgPath: any,
    @Inject(MOVIE_URL) public movieUrl: any
  ) {}
  ...
  getFilms () {
    return this.films.slice(0,5);
  }

  getPopularFilms (page?: number) {
    return this.http.get(`${this.movieUrl._desc}/popular?page=${page}${this.params}`)
  }

  searchPopularFilms (searchQuery: string, page?: number) {
    return this.http.get(`${this.searchUrl}/movie?page=${page}${this.params}&query=${searchQuery}`)
  }
  ...
  searchPopularActors (searchQuery: string, page?: number) {
    return this.http.get(`${this.searchUrl}/person?page=${page}${this.params}&query=${searchQuery}`)
  }

  getFilmById(filmId: number) {
    return this.http.get(`${this.apiUrl}/movie/${filmId}?api_key=${this.apiKey}&language=ru-RU`)
  }

  getActorById(personId: number) {
    return this.http.get(`${this.apiUrl}/person/${personId}?api_key=${this.apiKey}&language=ru-RU`)
  }

  markFavouriteMovie(accountId: string | null,  sessionId: string | null, filmId: string, isFav: boolean) {
    const options = {
    ...
    return this.http.post(`${this.apiUrl}/account/${accountId}/favorite?api_key=${this.apiKey}&session_id=${sessionId}`, options)
  }

  getFavoriteMovies(accountId: string | null, sessionId: string | null) {
    return this.http.get(`${this.apiUrl}/account/${accountId}/favorite/movies?api_key=${this.apiKey}&session_id=${sessionId}&language=ru-RU`)
  }

  getMovieWatchlist(accountId: string | null, sessionId: string | null) {
    return this.http.get(`${this.apiUrl}/account/${accountId}/watchlist/movies?api_key=${this.apiKey}&session_id=${sessionId}&language=ru-RU`)
  }

  addToWatchlist(accountId: string | null, sessionId: string | null, filmId: string, isWatch: boolean) {
    ...
    return this.http.post(`${this.apiUrl}/account/${accountId}/watchlist?api_key=${this.apiKey}&session_id=${sessionId}&language=ru-RU`, options)
  }

  //https://api.themoviedb.org/3/account?api_key=<<api_key>>

  getAccountDetail(sessionId: string | null) {
    return this.http.get(`${this.apiUrl}/account?api_key=${this.apiKey}&session_id=${sessionId}&language=ru-RU`)
  }
